...
## 6.  Write a CTE to calculate the total revenue for each products. (Revenues = Price × Quantity), and return only products where  revenue > 3000.
...
## 7. Create a view named vw_CategorySummary that shows: Category, TotalProducts, AveragePrice.
...
## 8. Create an updatable view containing ProductID, ProductName, and Price. Then update the price of ProductID = 1 using the view.
...
## 10. Create an AFTER DELETE trigger on the  Products table that archives deleted product rows into a new table ProductArchive . 
